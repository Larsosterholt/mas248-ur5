...
class MoveGroupPythonInterfaceTutorial(object):
    ...
    def __init__(self):
        ...
        self.poseArray_publisher = rospy.Publisher("waypoints", geometry_msgs.msg.PoseArray, queue_size=20)
    ...
    def process_image(self, image_path="src/mas248project/scripts/image_2.jpg"):
        ...
        for cont in contours:
            ...
            x_coord.extend(pixel_pose[:,1])  ## why 1 and not 0?? I don't know why it mirrror the image???
        ...
        #img.shape --> 210x290 (A4 size)
        x_offset = 0.3
        y_offset = -0.2
        x_coord = (-min(x_coord) + x_coord)/(max(x_coord)-min(x_coord))*0.20 + x_offset;#0.210
        y_coord = (-min(y_coord) + y_coord)/(max(y_coord)-min(y_coord))*0.20 + y_offset;#0.290

        #x_coord[:] = [x/ img.shape[0]*0.210 + x_offset for x in x_coord]
        #y_coord[:] = [y/ img.shape[1]*0.290 + y_offset for y in y_coord] 
    ...
    def add_floor(self):
        floor_pose = [0.0, 0.0, -0.225, 0, 0, 0, 1]
        floor_dimensions = [2, 2, 0.5]

        self.add_box_object("floor", floor_dimensions, floor_pose)

        print("============ Added one obstacle to the scene!!")

    def add_four_walls(self):
        back_wall_pose = [-0.7, 0, 0, 0, 0, 0, 1]
        back_wall_dimensions = [0.2, 2, 2]

        front_wall_pose = [1, 0, 0, 0, 0, 0, 1]
        front_wall_dimensions = [0.2, 2, 2]

        right_wall_pose = [0, -0.7, 0, 0, 0, 0, 1]
        right_wall_dimensions = [2, 0.2, 2]

        left_wall_pose = [0, 0.7, 0, 0, 0, 0, 1]
        left_wall_dimensions = [2, 0.2, 2]

        self.add_box_object("back_wall", back_wall_dimensions, back_wall_pose)
        self.add_box_object("front_wall", front_wall_dimensions, front_wall_pose)
        self.add_box_object("right_wall", right_wall_dimensions, right_wall_pose)
        self.add_box_object("left_wall", left_wall_dimensions, left_wall_pose)
    ...
    def go_to_pose_goal(self):
        ...
        self.x_coord_vec, self.y_coord_vec, self.z_coord_vec = self.process_image()
        ...
        pose_goal.position.x = self.x_coord_vec[0]
        pose_goal.position.y = self.y_coord_vec[0]
        pose_goal.position.z = self.z_coord_vec[0]
    ...
    def plan_cartesian_path(self, scale=1):
        ...
        for idx in range(len(self.x_coord_vec)):
            wpose.orientation.x = 0.0
            wpose.orientation.y = 0.0
            wpose.orientation.z = 0.0
            wpose.orientation.w = 1.0
            wpose.position.x =  self.x_coord_vec[idx]         
            wpose.position.y = self.y_coord_vec[idx] 
            wpose.position.z = self.z_coord_vec[idx] 
            waypoints.append(copy.deepcopy(wpose))
        ...
        (plan, fraction) = move_group.compute_cartesian_path(
            waypoints, 0.01, 0.0, True  # waypoints to follow  # eef_step
    ...
    def add_box(self, timeout=4):
        ...
        box_pose.header.frame_id = "tooltip"
